...
// Función para dibujar una parábola utilizando el algoritmo de Bresenham
void parabola_bresenham(int h, int k, int a) {
    int x = 0;
    int y = 0;
    int d = 1 - 2 * a; // Decisión inicial

    glBegin(GL_POINTS);

    // Fase 1: Mientras x crece, y también crece de forma controlada
    while (2 * a * x < y + 1) {
        glVertex2f(x + h, y + k);    // Dibuja la mitad positiva
        glVertex2f(-x + h, y + k);   // Dibuja la mitad reflejada en el eje Y

        if (d < 0) {
            d += 2 * x + 3;  // Actualiza la decisión
        }
        else {
            d += 2 * (x - y) + 5;
            y++;  // Incrementa y más rápido en esta región
        }
        x++;
    }

    // Fase 2: A medida que y crece más rápido que x, cambia el criterio
    d = a * (x + 0.5) * (x + 0.5) - (y + 1); // Recalcula el valor de la decisión

    while (y < 200) { // Limitar el crecimiento de y para evitar que se salga de la pantalla
        glVertex2f(x + h, y + k);    // Dibuja la mitad positiva
        glVertex2f(-x + h, y + k);   // Dibuja la mitad reflejada en el eje Y

        if (d > 0) {
            d -= 2 * y + 2;  // Actualiza la decisión
        }
        else {
            d += 2 * (x + 1) * a;
            x++;
...
void display(void) {
    ...
    glPointSize(2);     // Fija el grosor de pixel = 2
